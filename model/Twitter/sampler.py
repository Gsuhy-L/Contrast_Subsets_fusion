...
class Sampler(nn.Module):
  ...
  def all_reconstruct(self,random_top_k_idxs):
      # data_copy = []
      # print("top-k:")
...
#如果
  ...
  def reconstruct1(self, top_k_idxs):
    # print(data.x.shape)
    # chain_graph = self.chain_graph(self.data.edge_index)
    all_random_top_k_idxs = []
    sub_data_copy = []
    ...
    sub_data_copy = self.all_reconstruct(random_top_k_idxs)
    return sub_data_copy
  ...
  def graph_sampling(self, att):
    ...
    for j in range(self.Gnum_m):
     ...
     #现在我们已经有了所有节点的卷积结果，接下来我们要吧不同图的node out
      ...
      sub_data = self.reconstruct1(top_k_idxs)
