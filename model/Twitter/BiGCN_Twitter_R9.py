...
from RDEA_model_R9 import Net1, Classfier1
...
def train_GCN(treeDic, x_test, x_train, model_config, UDdroprate,lr, weight_decay,patience,n_epochs,batchsize,dataname,iter,con_lr):
    # model = Net(5000,64,64,model_config,device).to(device)
    unsup_model = Net1(64, 3, con_lr).to(device)
    unsup_epoch = 0
    for unsup_epoch in range(25):
        ...
        for Batch_data, Knowledge_data in tqdm_train_loader:
            optimizer.zero_grad()
    ...
    # with open('./data/' + dataname + '/' + dataname + '_vocab.pkl', 'rb') as vocab_file:
    #     vocab_dic = pickle.load(vocab_file)
    # len_vocab_dic = len(vocab_dic) + 1
    raw_word_vec_file = './data/' + dataname + '/raw_word2vec.npy'
    w2vec = np.load(raw_word_vec_file)
    w2vec = th.from_numpy(w2vec)
    w2vec = w2vec.float()
    model = Classfier1(64*3,64,4, w2vec).to(device)
    ...
    for epoch in range(n_epochs):
        ...
        for Batch_data ,Knowledge_data in tqdm_train_loader:
            ...
            Knowledge_data.to(device)
            _, Batch_embed = unsup_model.encoder(Batch_data.x, Batch_data.edge_index, Batch_data.batch, Batch_data)
            ...
            out_data, out_labels= model(Batch_embed, Batch_data,Knowledge_data, states = 'train')
            ...
            #out_noise_scloss = SCL_model(out_data, noise_x_, Batch_data.y)

            # noise_scloss = SCL_model(copy_out_data,copy_out_data , Batch_data.y)
            # con_loss = (out_data_scloss+out_noise_scloss+noise_scloss)/3
            #con_loss = out_data_scloss

            #这里对比的嵌入是,这种对比方法指的是同一个节点或/图在不同的视图中被视为正对，
            loss=finalloss#+0.1*con_loss+con_lr*out_noise_scloss#contrast_lr*loss_IntraGraph
            opt.zero_grad()
            loss.backward()
        ...
        for Batch_data,Knowledge_data in tqdm_test_loader:
            Batch_data.to(device)
            Knowledge_data.to(device)
            ...
            out_labels= model(Batch_embed, Batch_data, Knowledge_data, states = 'test')
...
def run_main(head_n):
    # set_seed(1)
    f = open('/home/ubuntu/PyProjects_gsuhyl/PyProjects/Contrast_Subsets_fusion/'+datasetname+'res_R9.txt','a')
    test_accs = []
    NR_F1 = []
    FR_F1 = []
    TR_F1 = []
