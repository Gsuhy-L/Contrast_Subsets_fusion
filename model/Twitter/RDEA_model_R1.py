...
from sampler_R1 import Sampler
...
class Classfier1(th.nn.Module):
    ...
    def forward(self, embed, data, states = 'train', datatype = "initdata"):
        #todo
        att_log_logit = self.extractor(embed,data.edge_index,data.batch)
        att = (att_log_logit).sigmoid()

        if states == 'train' and datatype=="initdata":
        ...
        if states == 'train' and datatype=="initdata":
            return sub_datas, x, out

        if states == 'train' and datatype=="subdata":
            return out
        if states == "test":
            return x
