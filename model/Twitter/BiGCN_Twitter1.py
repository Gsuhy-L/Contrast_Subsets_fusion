...
from RDEA_model import Net1, Classfier1
from SCL2 import SCL
...
def train_GCN(treeDic, x_test, x_train, model_config, UDdroprate,lr, weight_decay,patience,n_epochs,batchsize,dataname,iter,contrast_lr):
    ...
    #
    #     optimizer = th.optim.Adam(unsup_model.parameters(), lr=float(lr), weight_decay=float(weight_decay))
    #     unsup_model.train()
    ...
    for epoch in range(n_epochs):
        ...
        for Batch_data in tqdm_train_loader:
            ...
            # print(Batch_data.y)
            # print(e)
            subdata,out_labels= model(Batch_embed, Batch_data, states = 'train')
            ...
            #loss_IntraGraph = Graph_loss(g1=Batch_embed_x, g2=Sub_batch_embed_x, batch=Batch_data.batch)
            SCL_model = SCL(0.1).to(device)
            loss_IntraGraph = SCL_model(Batch_embed_x,Sub_batch_embed_x,Batch_data.y)

            finalloss=F.nll_loss(out_labels,Batch_data.y)
            #这里对比的嵌入是
            loss=finalloss+contrast_lr*loss_IntraGraph
            opt.zero_grad()
            loss.backward()
...
def run_main(contrast_lr):
    # set_seed(1)
    f = open('/home/ubuntu/PyProjects_gsuhyl/PyProjects/Contrast_Subsets_fusion/'+datasetname+'res.txt','a')
    test_accs = []
    NR_F1 = []
    FR_F1 = []
    TR_F1 = []
