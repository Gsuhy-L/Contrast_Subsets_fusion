...
class GATv1Layer(GeneralGATLayer):
    ...
    def get_x_r(self, x):
        # print(self.lin_r(x).view(-1, self.heads, self.out_channels).shape)
        # print((self.lin_r(x).view(-1, self.heads, self.out_channels) * self.att_r).shape)
    ...
    def compute_score(self, x_i, x_j, index, ptr, size_i):
        # print(e)
        # print(x_i.shape)
        # print(torch.sum(x_i + x_j, dim=-1, keepdim=True).shape)
        # print(e)
        x_dp = einsum('ijk,ijk->ij', x_j, x_i)
        x_dp = x_dp.unsqueeze(2)
        x_go = torch.sum(x_i + x_j, dim=-1, keepdim=True)

        x_go = einsum('ehk,ehk->ehk', x_go, torch.sigmoid(x_dp))  # change here
        alpha = F.leaky_relu(x_go, self.negative_slope)
