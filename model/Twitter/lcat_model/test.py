...
#这些参数卸载一起是为了方便，但实际上并不是给这个用的，这个只能用GCN和GAT
# test_lcat_module_GAT_GCN(out_channels=32, heads=4, heads_aggr='concat', add_self_loops=True, mode= 'gcn', att_version='v2', dataset=dataset())
...
#第二次运算都是GAT，第一次运算看是使用了GAT，还是GCN，还是CAT,LCAThe CATbutong
#这里便CAT和LCAT的区别在于两个LAMBA不同，一个不可以更新，一个可以更新
test_lcat_module_GAT_GCN(out_channels=32, heads=4, heads_aggr='concat', add_self_loops=True, mode='lcat', att_version='v1', dataset=dataset())
