...
from RDEA_model_R6 import Net1, Classfier1
...
def train_GCN(treeDic, x_test, x_train, model_config, UDdroprate,lr, weight_decay,patience,n_epochs,batchsize,dataname,iter,con_lr):
    ...
    unsup_model = Net1(64, 0.8).to(device)
    unsup_epoch = 0
    ...
    model = Classfier1(64*2,64,4).to(device)
    ...
    for epoch in range(n_epochs):
        ...
        for Batch_data in tqdm_train_loader:
            ...
            # out_noise_scloss = SCL_model(out_data, copy_out_data, Batch_data.y)
            ...
            loss=finalloss#+con_lr*con_loss#+contrast_lr*loss_CrossGraph#contrast_lr*loss_IntraGraph
            opt.zero_grad()
            loss.backward()
...
def run_main(head_n):
    set_seed(1)
    f = open('/home/ubuntu/PyProjects_gsuhyl/PyProjects/Contrast_Subsets_fusion/'+datasetname+'res_R6.txt','a')
    test_accs = []
    NR_F1 = []
    FR_F1 = []
    TR_F1 = []
