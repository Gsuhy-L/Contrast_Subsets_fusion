    ...
    def graph_sampling(self, att):
        ...
        for j in range(self.Gnum_m):
            ...
            for i in range(max(self.data.batch) + 1):
                # 他的意思是找到节点
                # print(self.nodes_num)
                selected_edges = self.sample_subset(att.index_select(1, torch.tensor(j).to(self.device)).index_select(0,
                                                                                                                      torch.tensor(
                                                                                                                          np.arange(
                                                                                                                              self.nodes_num[
                                                                                                                                  i].cpu(),
                ...
                top_k_idx = np.sort(np.array(torch.argsort(selected_edges).cpu())[-self.subNnum_k[i]:]) + \
